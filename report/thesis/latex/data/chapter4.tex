...
本系统通过第三方统一API网关接入多种大语言模型，模型名称通过\texttt{LLM\_MODEL}环境变量或\texttt{--model}命令行参数指定。
...
本章系统地定义了本文全部实验的设计方案。实验以VerilogEval-Human和RTLLM\_STUDY\_12为评估基准，使用三个不同能力等级的大语言模型，通过七组递进式实验回答关于反馈机制有效性、价值归因、信息量影响、组织方式和提示词策略的研究问题。这些设计为第5章的实验结果分析奠定了方法论基础。
