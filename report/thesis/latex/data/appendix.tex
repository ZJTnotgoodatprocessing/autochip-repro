...
% TODO: 根据 appendix_plan.md 补充完整的模块索引表
...
模块 & 文件路径 \\
\midrule
反馈循环控制器 & \texttt{src/loop\_runner.py} \\
提示词构建器 & \texttt{src/prompt\_builder.py} \\
Verilog执行器 & \texttt{src/verilog\_executor.py} \\
评分器 & \texttt{src/ranker.py} \\
RTLLM加载器 & \texttt{src/rtllm\_loader.py} \\
