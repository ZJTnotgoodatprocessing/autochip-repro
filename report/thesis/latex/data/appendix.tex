% !Mode:: "TeX:UTF-8"
\chapter{实验配置与运行命令}

本附录记录了本文实验使用的典型运行命令模板和关键配置。

\section{正式实验运行命令}

\begin{verbatim}
# Zero-shot
python scripts/run_rtllm_subset.py \
  --model gpt-5.4 --subset study12 \
  --mode zero-shot

# Feedback (default: k=3, iter=5, succinct)
python scripts/run_rtllm_subset.py \
  --model gpt-5.4 --subset study12 \
  --mode feedback

# Ablation (zero-shot + retry-only + feedback)
python scripts/run_rtllm_subset.py \
  --model gpt-5.4 --subset study12 \
  --mode ablation

# Granularity (L0-L4)
python scripts/run_rtllm_subset.py \
  --model gpt-5.4 --subset study12 \
  --mode granularity

# Multiturn (ST/MT/CO conditions)
python scripts/run_rtllm_subset.py \
  --model gpt-5.4 --subset study12 \
  --mode multiturn

# Prompt strategy (P0-P3, ZS+FB each)
python scripts/run_rtllm_subset.py \
  --model gpt-5.4 --subset study12 \
  --mode prompt_strategy
\end{verbatim}

\section{环境配置}

实验环境：Python 3.11，Icarus Verilog 12.0，Ubuntu 22.04。API通过第三方统一网关接入。

\section{核心代码模块索引}

\begin{table}[htbp]
\centering
\caption{系统核心代码模块索引}
\begin{tabular}{ll}
\toprule
模块 & 文件路径 \\
\midrule
反馈循环控制器 & \texttt{src/feedback/loop\_runner.py} \\
提示词构建器 & \texttt{src/feedback/prompt\_builder.py} \\
Verilog执行器 & \texttt{src/runner/verilog\_executor.py} \\
评分器 & \texttt{src/ranking/ranker.py} \\
RTLLM加载器 & \texttt{src/runner/rtllm\_loader.py} \\
VerilogEval加载器 & \texttt{src/runner/verilogeval\_loader.py} \\
LLM客户端 & \texttt{src/llm/client.py} \\
Verilog提取器 & \texttt{src/utils/extract\_verilog.py} \\
实验产物管理 & \texttt{src/utils/artifacts.py} \\
正式实验运行脚本 & \texttt{scripts/run\_rtllm\_subset.py} \\
RTLLM兼容性扫描 & \texttt{scripts/scan\_rtllm\_iverilog.py} \\
代码审计 & \texttt{scripts/audit\_project.py} \\
数据审计 & \texttt{scripts/audit\_data.py} \\
\bottomrule
\end{tabular}
\end{table}

\chapter{补充实验图表}

以下图表为第5章实验结果的逐题细节补充，供需要深入了解各题目表现差异的读者参考。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figure/fig_per_problem_matrix.pdf}
\caption{RTLLM\_STUDY\_12逐题结果矩阵}
\label{fig:per_problem_matrix}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figure/fig_feedback_gain.pdf}
\caption{各模型Feedback增益对比}
\label{fig:feedback_gain}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figure/fig_granularity_matrix.pdf}
\caption{反馈粒度逐题结果矩阵}
\label{fig:granularity_matrix}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figure/fig_multiturn_matrix_v2.pdf}
\caption{多轮对话反馈v2逐题矩阵}
\label{fig:multiturn_matrix}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figure/fig_prompt_strategy_matrix.pdf}
\caption{提示策略逐题结果矩阵}
\label{fig:prompt_strategy_matrix}
\end{figure}
