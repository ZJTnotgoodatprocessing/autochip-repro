...
## 附录 B：v10 文字小修记录（2026-05-15）

在 v10 PDF 与图号修复（附录 A）落地之后，又对第 5 章做了一轮**纯文字层面**的稳妥化修订。本轮**未改任何实验数据、未新增/重跑实验、未改正文结构**，只调整三处具体措辞。

### B.1 修正 §5.10 错字 `可萐取` → `可参考`

**位置**：`@c:/Users/17819/autochip-repro/report/thesis/latex/data/chapter5.tex:348`（§5.10 本章小结）

**原文**：

> ...并从成本收益、模型选型、反馈粒度三个角度提供了**可萐取**的部署建议。

**改后**：

> ...并从成本收益、模型选型、反馈粒度三个角度提供了**可参考**的部署建议。

"萐"是生僻字，明显是输入错字。

### B.2 §5.10 小结中"证实了" → "支持了"（过强表述稳妥化）

**位置**：同上行 §5.10。

**原文**：

> ...GPT-5.4增益最大（+33个百分点），**证实了**反馈循环在不同难度和模型上的基本有效性。

**改后**：

> ...GPT-5.4增益最大（+33个百分点），**支持了**反馈循环在不同难度和模型上的基本有效性。

"证实"语义过强，本科论文样本规模有限的实验结论用"支持"更稳妥，与 §5.10 末尾的"局限性"段也更一致。

### B.3 反馈粒度结论稳妥化（不再对所有模型笼统说"倒 U 型"）

#### B.3.1 §5.5 正文（`chapter5.tex:152`）

**原文**：

> 结果如表\ref{tab:granularity}所示，**呈现出倒U型趋势**：通过率随反馈信息量的增加先升后降，L2（Compile-only）和L3（Succinct）达到峰值，而L4（Rich）反而出现下降。

**改后**：

> 结果如表\ref{tab:granularity}所示，**反馈粒度存在较明显的最佳区间**：通过率随反馈信息量的增加先上升，在L2（Compile-only）和L3（Succinct）附近达到较好水平，随后在过量反馈条件下可能下降。**GPT-5.4 的曲线呈现较明显的倒 U 型趋势，Sonnet 4.6 则在 L2 后回落并趋于平稳。**

新写法保留了"倒 U 型"这一关键形态描述，但**明确限定为 GPT-5.4**；对 Sonnet 4.6 改用"在 L2 后回落并趋于平稳"，更贴合表 5.5 的实际数据（Sonnet 在 L3/L4 并未明显回升再下降）。

#### B.3.2 表 5.10（`chapter5.tex:336`，结论汇总）

**原文**：`粒度实验 | L0--L4 | 倒U型，L2最稳健`

**改后**：`粒度实验 | L0--L4 | 反馈粒度存在最佳区间，L2较稳健`

#### B.3.3 §5.10 小结（`chapter5.tex:348`）

**原文**：

> ...反馈粒度实验**发现倒U型边界且L2编译反馈最为稳健**...

**改后**：

> ...反馈粒度实验**显示存在最佳区间，L2编译反馈较为稳健**...
...
| LaTeX Warning | 0 |
| BibTeX warning | 0 |
| v10 PDF 大小 | 2131.8 KB（首版 2142.9 → 修图后 2131.9 → 本轮 2131.8） |
| v9 PDF | **未触碰**（仍为 1970.6 KB / 58 页 / 0:01:11） |

### B.5 chapter6 结论章同步修订（追加于 2026-05-15 00:53）

完成 §5.5 / §5.10 / 表 5.10 的稳妥化后，发现 `@c:/Users/17819/autochip-repro/report/thesis/latex/data/chapter6.tex:18`（结论章）仍写"粒度实验呈现倒 U 型趋势"，对所有模型笼统。经作者确认后一并修复，让 chapter6 与 chapter5 口径完全一致。
...
1. 把"粒度实验呈现倒 U 型趋势"由对所有模型笼统的陈述，改为仅对 GPT-5.4 的限定描述（与 chapter5 §5.5 完全一致）。
2. 把"是最稳健的选择"软化为"较为稳健"，与 chapter5 表 5.10 行"L2 较稳健"用词一致。

修订后再次重新编译，仍为 63 页 / 0 undefined / 0 Overfull / 0 LaTeX warning / 0 BibTeX warning；v10 PDF 刷新为 2131.8 KB（2026-05-15 00:52:56），v9 PDF 仍未触碰（1970.6 KB / 0:01:11）。
