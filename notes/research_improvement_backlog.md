...
| 多轮对话实验 | ✅ 完成 | MT 对 GPT 有害(−50pp)，对 Sonnet 中性 |
...
### 5. 多轮对话反馈 [低优先级 — 已实验验证为负面]

- **研究价值**: ★★☆☆☆ — 实验证明 multi-turn 对 GPT 有害（83%→33%），对 Sonnet 无益（67%→67%）
- **实验状态**: ✅ 已在 Multiturn-6 子集上完成对比实验
- **关键发现**: single-turn 的"清洁重置"和 k>1 候选多样性是关键优势，multi-turn 的"错误路径锁定"效应严重
- **实现成本**: 已完成 — `run_multiturn_feedback_loop` 已集成
- **建议优先级**: 低 — 实验已排除此方向的价值。在论文中作为"负面实验"报告
- **论文价值**: 可作为"我们系统地验证了多轮对话反馈，结果表明单轮方法更优"的论据
